    ...
    public List<BaseModel> search(SearchTO searchTO) {
        if (searchTO.getSearchType() == 2) {
            switch (getSearchField(searchTO.getSearchField())) {
                ...
                case TAGS:
                    return users.stream().filter(user -> CollectionUtils.isEmpty(user.getTags().stream().filter(tag -> StringUtils.containsIgnoreCase(tag, searchTO.getSearchKey())).collect(Collectors.toList()))).collect(Collectors.toList());
                default:
