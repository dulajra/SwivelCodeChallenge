                ...
                case TAGS:
                    return organizations.stream().filter(org ->
                            CollectionUtils.isNotEmpty(org.getTags()) &&
                                    CollectionUtils.isNotEmpty(org.getTags().stream().filter(tag -> StringUtils.containsIgnoreCase(tag, searchTO.getSearchKey())).collect(Collectors.toList()))).collect(Collectors.toList());
                default:
